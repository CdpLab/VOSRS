...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        ratios = max_scales / min_scales
        scales_loss_sum = torch.clamp(ratios, min=r).sum() - r * scales.shape[0]
        scales_loss = scales_loss_sum / scales.shape[0]

        loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (1.0 - ssim(image, gt_image)) + scales_loss
        loss.backward()
