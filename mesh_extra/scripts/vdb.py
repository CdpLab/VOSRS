...
def main():
    ...
    if os.path.isdir(model_path):
        ...
        work_dir = os.path.join(model_path, "renders", "iteration_{}".format(
            loaded_iter)) if args.output_dir is None else args.output_dir
        ...
        work_dir = os.path.join(os.path.dirname(model_path),
                                os.path.basename(model_path)[:-4]) if args.output_dir is None else args.output_dir
        pcd.load(model_path)
    else:
    ...
    if args.source_path.endswith(".json"):
        ...
        cameras = dataset.all_cameras  # pass
    ...
    for camera in tqdm(cameras[::3]):
        ...
        filename = "output_scene.vdb"
        vdb_volume.save(filename)
